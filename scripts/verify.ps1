...
if ($Status.queue.completed -ne 1) {
 Write-Error "Error: Scenario 1 failed"
}
...
if ($Status.queue.dead -ne 1) {
 Write-Error "Error: Scenario 2 failed"
}
...
if ($Lines -ne 5) {
 Write-Error "Error: Scenario 3 failed"
}
...
if ($Status.queue.dead -ne 2) {
 Write-Error "Error: Scenario 4 failed"
}
...
if ($Status.queue.pending -ne 1) {
 Write-Error "Error: Scenario 5 failed"
}
